...
class TestXGBoostDailyAnalysis(unittest.TestCase):
    ...
    def test_metrics(self):
        ...
        model, predictions = xgboost_daily_analysis(self.data)
        _, X_test, _, y_test = train_test_split(self.data[:-1], self.data[1:], test_size=0.2, random_state=42)
    ...
    def test_real_data(self):
        ...
        future_data, _ = extract_values('Monthly FTSE Data - New.xlsx',companies, '01/01/2023','01/03/2023')
        model, _ = xgboost_daily_analysis(data, justTrain=True)
        predictions = justTest(model, data[-1], 5)
        #plot_data(future_data, predictions, data)
        self.assertIsNotNone(model)
        self.assertIsNotNone(predictions)
    
    def test_justTest_predictions_length(self):
        # Test if justTest returns the correct number of predictions
        model, _ = xgboost_daily_analysis(self.data, 1, justTrain=True)
        predictions = justTest(model, self.data[-1], 10, 1)
    ...
    def test_justTest_predictions_values(self):
        ...
        model, _ = xgboost_daily_analysis(self.data, 2, justTrain=True)
        predictions = justTest(model, self.data[-2:].T[0], 10)
